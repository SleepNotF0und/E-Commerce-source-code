<?php 
    ...
    $sql_select = "SELECT * FROM gears_images;";
    $result = $connect->query($sql_select);

    if ($result->num_rows > 0 && $result->num_rows <= 20) {            #must know the capasity of the page 
        while($row = $result->fetch_assoc()) {
            echo "<div class=box>";
                echo "<center>"; echo "<img  class=ftched_img src='uploads/gears/" .$row['Images']. "' alt='' />"; echo "</center>";
                echo "<br>";
    ...
    elseif ( $result->num_rows > 20 ) {
        ...
        $ID = $row['Id'];
        $connect->query( "DELETE FROM gears_images WHERE Id = $ID ;");
    }
    else {
        echo "No Products in the Data Base";
    }
?>
